  ...
  switch (item.type) {
    ...
    case WriteQueueType.CASH_SESSION_OPEN:
      return openCashSession(
        supabase,
        item.payload as Parameters<typeof openCashSession>[1],
      );
    case WriteQueueType.CASH_SESSION_CLOSE:
      return syncCashSessionClose(
        supabase,
        item.payload as CashSessionClosePayload,
      );
    ...
    case WriteQueueType.TABLE_UPDATE_STATUS:
      return syncTableUpdateStatus(
        supabase,
        item.payload as TableUpdateStatusPayload,
      );
...
async function resolveOpenSessionId(
  ...
  if (!data?.id)
    throw new Error("No open cash session found for this location");
...
async function syncOrderWrite(
  ...
  const sendToKds =
    order.kds_enabled !== false && shopFlags?.kds_enabled !== false;
  const itemKdsStatus = sendToKds ? KdsStatus.PENDING : KdsStatus.SERVED;
