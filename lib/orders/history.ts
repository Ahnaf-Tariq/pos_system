...
export async function fetchOrders(
  ...
  const [{ data: items }, { data: tables }, { data: profiles }] = await Promise.all([
    supabase
      .from('order_items')
      .select('order_id')
      .eq('user_id', userId)
  ...
  let rows: OrderListRow[] = orders.map((order) => ({
    ...
    table_label: order.table_id ? tableLabelById.get(order.table_id) ?? null : null,
...
export async function fetchOrderDetail(
  ...
  const [{ data: items }, { data: table }, { data: profile }] = await Promise.all([
    supabase
      .from('order_items')
  ...
  const detailItems: OrderDetailItem[] = (items ?? []).map((row) => ({
    ...
    menu_item_name: (row.menu_items as { name?: string } | null)?.name ?? 'Item',
  }))
