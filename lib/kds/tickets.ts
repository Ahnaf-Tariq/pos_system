...
export async function fetchKdsTickets(
  ...
  const [{ data: items, error: itemsError }, { data: tables }] =
    await Promise.all([
      supabase
        .from("order_items")
...
export function deriveTicketStage(items: KdsTicketItem[]): KdsStatus {
  ...
  if (statuses.every((status) => status === KdsStatus.SERVED))
    return KdsStatus.SERVED;
  if (
    statuses.every(
      (status) => status === KdsStatus.READY || status === KdsStatus.SERVED,
    )
  )
    return KdsStatus.READY;
  if (
    statuses.some(
      (status) => status === KdsStatus.PREPARING || status === KdsStatus.READY,
    )
  )
...
export async function bumpTicket(
  ...
  if (next === KdsStatus.READY && ticket.order.status !== OrderStatus.PAID) {
...
export async function markPaidTicketServed(
  supabase: SupabaseClient,
  userId: string,
  orderId: string,
) {
...
export async function bumpItem(
  ...
  if (stage === KdsStatus.READY && ticket.order.status !== OrderStatus.PAID) {
