...
export function PaymentModal({
  ...
  function handleConfirm() {
    ...
    onConfirm({
      ...
      amountTendered: method === PaymentMethod.CASH ? amountTendered : grandTotal,
    })
    onOpenChange(false)
    setTendered('')
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Total due{' '}
            <span className="money text-base">{formatMoney(grandTotal, currency)}</span>
          </DialogDescription>
        </DialogHeader>

        <div className="flex gap-2">
          {([PaymentMethod.CASH, PaymentMethod.CARD, PaymentMethod.SPLIT] as const).map(
            (value) => (
              <Button
                key={value}
                type="button"
        ...
        {method === PaymentMethod.CASH ? (
          <div className="space-y-3">
            <div className="rounded-md border border-border bg-secondary/40 px-3 py-2">
              ...
              <p className="money text-xl">{formatMoney(amountTendered || 0, currency)}</p>
              <p className="mt-1 text-sm text-muted-foreground">
                Change: <span className="text-primary">{formatMoney(change, currency)}</span>
              </p>
            </div>
          ...
          <p className="text-sm text-muted-foreground">
            {method === PaymentMethod.CARD
              ...
              : 'Split payment recorded as settled for V1. Detailed split lines come later.'}
          </p>
        )}
