...
export function StaffDetailView({
  ...
  const refresh = useCallback(
    async (opts?: { silent?: boolean }) => {
      ...
      try {
        ...
        const next = await fetchStaffDetail(supabase, userId, staffId, payBasis);
  ...
  useRealtimeRefresh({
    userId,
    tables: ["salary_payments", "staff_members"],
      ...
      <section className="relative overflow-hidden rounded-xl border border-border bg-card">
        ...
        <div className="relative flex flex-col gap-6 p-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="grid gap-3 sm:min-w-[220px]">
            <div className="rounded-lg border border-border/80 bg-background/50 px-4 py-3 backdrop-blur-sm">
